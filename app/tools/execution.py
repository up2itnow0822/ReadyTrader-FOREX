...
def pre_trade_check(
    ...
    sentiment, market, notional_usd, position_units, exposure_added_units, market_price}. place_stock_order runs it before anything is
    proposed or executed, and the approval API (app/api_server.py) runs it again at execution
    time, because a proposal can wait up to its expiry while the market moves.
    ...
    if notional is None:
        ...
        refusal = f"Could not read the account's equity ({'paper' if settings.PAPER_MODE else exchange}) for the position-size check."
...
def place_market_order(symbol: str, side: str, amount: float, rationale: str = "", sentiment_score: float | None = None) -> str:
...
def place_limit_order(symbol: str, side: str, amount: float, price: float, rationale: str = "", sentiment_score: float | None = None) -> str:
